...
#define DATE_MESSAGE_SIZE 30
 ...
 void fat_fuse_log_activity(char *operation_type, fat_file target_file)
 {
     ...
     fat_tree_node log = fat_tree_node_search(tree, "/fs.log");
...
static void fat_fuse_log_init(void){  
    fat_volume vol = get_fat_volume();
    fat_tree tree = vol->file_tree;
    if(fat_tree_node_search(tree, "/fs.log") == NULL){
        
        if (fat_fuse_mknod("/fs.log", 0, 0) != 0) {
            DEBUG("Aneble to creat " "/fs.log");
            return;
        }
        /*
        fat_tree_node log = fat_tree_node_search(tree, LOG_FILE);
        fat_file log_file = fat_tree_get_file(log);
        fat_file parent = fat_tree_get_parent(log);
